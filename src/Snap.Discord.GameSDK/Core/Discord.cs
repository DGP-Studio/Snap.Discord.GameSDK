using ABI.Snap.Discord.GameSDK.Core;
...
public class Discord : IDisposable
{
    ...
    private nint EventsPtr;

    private DiscordEvents Events;

    private nint ApplicationEventsPtr;
    ...
    public unsafe Discord(long clientId, ulong flags)
    {
        ...
        var result = DiscordCreate(3, &createParams, out MethodsPtr);
        if (result != Result.Ok)
        {
            Dispose();
